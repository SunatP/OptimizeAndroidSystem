...
if [ -z $MAT ]; then
  ...
  if $FUNCTION; then
    ...
    ui_print "   Vol+ = mat(V4A), Vol- = ARISE"
    if $FUNCTION; then
      MAT=true
    else
      NEW=true
...
if $MAT; then
  ui_print "   ViPER4ARISE will be installed"
  ...
  sed -ri "s/name=(.*)/name=\1 ARISE/" $INSTALLER/module.prop
  sed -i "s/author=.*/author=Sunat.P , Topjohnwu/" $INSTALLER/module.prop
  $LATESTARTSERVICE && sed -i 's/<ACTIVITY>/com.pittvandewitt.viperfx/g' $INSTALLER/common/service.sh
elif $NEW; then
  ui_print "   ViPER4ARISE $VER will be installed"
